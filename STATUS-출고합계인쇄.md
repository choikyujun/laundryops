...
## 현재 단계
**개발 진행 중.** 태스크 0~4 완료. 다음: 태스크 5(완료 정의 대조 + 최종 점검).
...
| 개발 (dev) | 진행 중 | 태스크 0~4 완료. features/outbound-compare.js 만 수정. app_v38.js·print-fix.js 무변경 확인 |
...
## 인쇄 검증 (Chrome PDF, 태스크 4)
`page.pdf({ printBackground: false })` 로 PDF 를 만들고 스트림을 풀어 색 연산자(`r g b rg`)를 직접 확인했다.

| 확인 | 결과 |
|---|---|
| 정상 배지 배경 `#d1fae5` | PDF 에 존재 ✓ |
| 확인 필요 배지 배경 `#fee2e2` | PDF 에 존재 ✓ |
| 표 머리 배경 `#f1f5f9` | 존재 ✓ |
| 글자색 `#065f46` / `#991b1b` | 존재 ✓ |
| **대조군** — `print-color-adjust` 규칙 제거 | 배지 배경 **사라짐**, 글자색은 남음 |

→ 그 한 줄이 배지 배경을 살리는 것이 실측으로 확인됨.
...
| 3개 | 1장 | 유지 |
| 40개 | 2장 | 유지 |
| 120개 (특수, 카테고리 12그룹) | 4장 | 유지 |

잘리지 않고 이어지며, `thead` 반복 설정으로 각 페이지에 머리 행이 나온다.
...
| 2026-09-11 | 인쇄는 `window.printObSummary` 전용 경로 | `window.printReport` 미변경 확인(재정의·호출 둘 다 없음). 기존 호출처 4곳 무영향 | 태스크 4 |
| 2026-09-11 | `* { print-color-adjust: exact !important }` + `-webkit-` 접두사 | **Chrome PDF 로 대조 검증:** 규칙이 있으면 `printBackground:false`(사장님이 "배경 그래픽" 체크 안 한 상태)에서도 배지 배경이 찍히고, 규칙을 빼면 사라진다. 글자색은 양쪽 다 남음 → 배경만 영향 | 태스크 4 |
| 2026-09-11 | 인쇄 시 `window.open` 전에 재조회하지 않음 | `await` 뒤에 `window.open` 을 부르면 팝업 차단에 걸린다. 화면에 그려진 `#obSummaryBody` 를 동기로 복제 | 개발 |
| 2026-09-11 | 인쇄 복제본에서 `<svg>` 제거 | 아이콘이 스프라이트 참조(`<use href="#i-...">`)라 인쇄 창엔 원본이 없어 빈 칸만 남는다 | 개발 |
| 2026-09-11 | 팝업 안 "N월 누계 대조" 제목은 인쇄에서 숨김(`.ob-sum-label`) | 머리말과 중복. 같은 줄의 "세탁 대기 N건" 배지는 유지 | 개발 |
| 2026-09-11 | `thead { display: table-header-group }` + `tr { page-break-inside: avoid }` | 품목이 많으면 여러 장으로 이어지는데 머리 행이 각 페이지에 반복되어야 읽힌다 | 개발 |
| 2026-09-11 | 누계 제목 월 표기 `09월` → `9월` | `month.slice(5)` 가 0 을 남겼다. 합계 버튼(`Number(...)`)과 표기 통일 | 개발 |
